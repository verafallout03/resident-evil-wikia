<div>
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Locaciones</h1>
        ...
        @forelse ($locations as $l)
            <a href="{{ route('locations-detail', $l['slug']) }}"
               class="group block overflow-hidden rounded-lg bg-white shadow hover:shadow-lg transition">
                ...
                @endif
                <div class="p-3">
                    <h3 class="font-semibold text-gray-800 truncate">{{ $l['name'] }}</h3>
                    @if (!empty($l['region']))
                        <p class="text-xs text-gray-500 truncate">{{ $l['region'] }}</p>
                    @endif
                </div>
            </a>
        @empty
            <div class="col-span-4 py-16 text-center text-gray-400">
                <p class="text-lg">No hay locaciones disponibles.</p>
            </div>
        @endforelse
    </div>

<x-app-layout>
    <div class="mb-4 flex items-center justify-between">
        <h1 class="text-2xl font-bold">Editar: {{ $location->name }}</h1>
    ...
    <form action="{{ route('admin.locations.update', $location) }}" method="POST"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="region" value="{{ old('region', $location->region) }}"
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full rounded border px-3 py-2">{{ old('description', $location->description) }}</textarea>
        </div>

    ...
    <form action="{{ route('admin.characters.store') }}" method="POST"
            ...
            <div>
                ...
                <select name="faction" class="w-full rounded border px-3 py-2">
                    ...
                    @foreach (['S.T.A.R.S.','B.S.A.A.','Umbrella','Neo-Umbrella','The Connections','Independent','Villain','Infected','Unknown'] as $f)
                        <option value="{{ $f }}" {{ old('faction') === $f ? 'selected' : '' }}>{{ $f }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="blood_type" value="{{ old('blood_type') }}" maxlength="5"
                       class="w-full rounded border px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium">Altura (cm)</label>
                <input type="number" name="height_cm" value="{{ old('height_cm') }}" min="50" max="300"
        ...
        <div>
            ...
            <textarea name="lore" rows="4"
                      class="w-full rounded border px-3 py-2">{{ old('lore') }}</textarea>
        </div>

<x-app-layout>
    <div class="mb-4 flex items-center justify-between">
        <h1 class="text-2xl font-bold">Editar: {{ $game->title }}</h1>
    ...
    <form action="{{ route('admin.games.update', $game) }}" method="POST"
        ...
        <div>
            ...
            <input type="text" name="slug" value="{{ old('slug', $game->slug) }}"
            ...
            <div>
                ...
                <select name="canon" class="w-full rounded border px-3 py-2 @error('canon') border-red-500 @enderror">
                    @foreach (['main' => 'Principal', 'spin-off' => 'Spin-off', 'remake' => 'Remake'] as $val => $label)
                        <option value="{{ $val }}"
                            {{ old('canon', $game->canon) === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <input type="text" name="cover_image" value="{{ old('cover_image', $game->cover_image) }}"
                   class="w-full rounded border px-3 py-2">
        </div>
        ...
        <div>
            ...
            <textarea name="synopsis" rows="4"
                      class="w-full rounded border px-3 py-2">{{ old('synopsis', $game->synopsis) }}</textarea>
        </div>
